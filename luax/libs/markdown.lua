...
local function highlight(lang, code)
    -- create tmp file with code
    local tmp_filename = os.tmpname()
    local ext = lang_to_ext[lang]
    if not ext then
        ext = lang
    end
    -- add matching extension to tmp_filename (just lang for now)
    tmp_filename = tmp_filename .. "." .. ext
    local tmp_file = io.open(tmp_filename, "w")
    if not tmp_file then
      error("Could not open tmp file: " .. tmp_filename)
    end
    tmp_file:write(code)
    tmp_file:close()
    ...
    return tree_sitter_output:read("*a")
end

local function code_block_highlight(text)
    -- find ``` blocks
    local pos = 1
    while true do
        local start, stop = text:find("```", pos)
        if not start then break end
        local endstart, endstop = text:find("```", stop+1)
        if not endstart then break end

        local block = text:sub(stop+2, endstart-1)
        -- treat first line after ``` as language
        local lang = block:match("^(.-)\n")
        -- remove first line from block
        block = block:gsub("^[^\n]*\n", "")
        -- replace block with highlighted block
        block = '<div class="code-block">' .. highlight(lang, block) .. '</div>'
        text = text:sub(1, start-1) .. block .. text:sub(endstop+1)
        code_block_ranges[#code_block_ranges+1] = {start, start+block:len()}
        pos = start + block:len()
    end
    return text
end
...
local function markdown(text)
   ...
   text = cleanup(text)
   text = protect(text)
   text = code_block_highlight(text)
